...
function AuthScreen({ onAuthenticated }) {
  ...
  return <div className="auth-page"><section className="auth-copy"><a className="logo" href="#"><span><Icon name="spark" size={17} /></span>SkillForge</a><div><p className="kicker">YOUR CAREER, CLARIFIED</p><h1>Start with what you know.<br /><i>Grow into what you want.</i></h1><p>Build a private career roadmap that adapts as your skills grow.</p></div><small>SkillForge · Career planning with direction</small></section><section className="auth-panel"><form className="auth-card" onSubmit={submit}><p className="kicker">WELCOME TO SKILLFORGE</p><h2>{mode === 'signup' ? 'Create your account' : 'Welcome back'}</h2><p>{mode === 'signup' ? 'Save your roadmap and track your growth.' : 'Sign in to continue your career plan.'}</p>{mode === 'signup' && <label>Name<input required value={form.name} onChange={event => setForm(current => ({ ...current, name: event.target.value }))} placeholder="Your name" autoComplete="name" /></label>}<label>Email<input required type="email" value={form.email} onChange={event => setForm(current => ({ ...current, email: event.target.value }))} placeholder="you@example.com" autoComplete="email" /></label><label>Password<span className="password-entry"><input required minLength="8" type={showPassword ? 'text' : 'password'} value={form.password} onChange={event => setForm(current => ({ ...current, password: event.target.value }))} placeholder="At least 8 characters" autoComplete={mode === 'signup' ? 'new-password' : 'current-password'} /><button className="password-toggle" type="button" onClick={() => setShowPassword(current => !current)} aria-label={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? 'Hide' : 'Show'}</button></span></label><button className="generate" disabled={loading}>{loading ? 'Please wait…' : mode === 'signup' ? 'Create account' : 'Sign in'} <Icon name="arrow" size={18} /></button>{error && <p className="form-error" role="alert">{error}</p>}<button className="auth-toggle" type="button" onClick={() => { setMode(current => current === 'signup' ? 'login' : 'signup'); setError(''); }}>{mode === 'signup' ? 'Already have an account? Sign in' : 'New to SkillForge? Create an account'}</button></form></section></div>;
}
...
function App() {
  ...
  const editProfile = update => {
    ...
    setRoadmap(null); setCompletedSteps([]); setChatMessages([]); setError('');
  };
  ...
  return <div className={`app theme-${theme}`}>
    <nav className="nav"><a className="logo" href="#top"><span><Icon name="spark" size={17} /></span>SkillForge</a><div className="nav-links"><a href="#assessment">Assessment</a><a href="#roadmap">Roadmap</a></div><div className="nav-actions"><button className="nav-icon" onClick={() => setTheme(current => current === 'light' ? 'dark' : 'light')} aria-label="Toggle light or dark theme" title="Toggle theme"><Icon name={theme === 'light' ? 'moon' : 'sun'} size={17} /></button><button className="nav-icon sign-out" onClick={() => { localStorage.removeItem('skillforge-auth'); setAuth(null); setProfileId(null); setRoadmap(null); }} aria-label="Sign out" title="Sign out"><Icon name="logout" size={17} /></button><div className="nav-avatar" title={auth.user.name}>{initials}</div></div></nav>
    <main id="top">
      ...
      <section className="assessment" id="assessment"><div className="section-heading"><div><p className="kicker">START HERE</p><h2>Build your skill profile</h2><p>Changing your goal or skills clears the old plan, so every roadmap and assistant answer stays accurate.</p></div><span className="step-badge">01 <small>of 02</small></span></div>
        <div className="assessment-grid"><form className="profile-card" onSubmit={event => { event.preventDefault(); generateRoadmap(); }}><label>Your name<input value={profile.name} onChange={event => editProfile({ name: event.target.value })} placeholder="e.g. Zoha Khawaja" autoComplete="name" /></label><label>Career goal<select value={profile.targetRole} onChange={event => editProfile({ targetRole: event.target.value })}>{roleOptions.map(role => <option key={role}>{role}</option>)}</select></label><label>Skills you already have</label><div className="skills-entry">{profile.skills.map(skill => <span className="skill-chip" key={skill}>{skill}<button type="button" onClick={() => editProfile({ skills: profile.skills.filter(item => item !== skill) })} aria-label={`Remove ${skill}`}><Icon name="close" size={13} /></button></span>)}<input value={skillInput} onChange={event => setSkillInput(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); addSkill(); } }} placeholder="Type a skill and press Enter" /></div><button className="add-skill" type="button" onClick={() => { addSkill(); setRoadmap(null); setCompletedSteps([]); setChatMessages([]); }}><Icon name="plus" size={15} /> Add skill</button><button className="generate" disabled={!canGenerate || loading}>{loading ? 'Creating your roadmap…' : 'Generate my roadmap'} <Icon name="arrow" size={18} /></button>{error && <p className="form-error" role="alert">{error}</p>}</form>
