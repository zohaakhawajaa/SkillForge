...
function App() {
  ...
  return <div className={`app theme-${theme}`}>
    <nav className="nav"><a className="logo" href="#top"><span><Icon name="spark" size={17} /></span>SkillForge</a><div className="nav-links"><a href="#assessment">Assessment</a><a href="#roadmap">Roadmap</a></div><div className="theme-switcher" role="group" aria-label="Choose a colour theme">{[['mono', 'Mono'], ['dusk', 'Dusk'], ['sage', 'Sage']].map(([value, label]) => <button key={value} className={theme === value ? 'active' : ''} onClick={() => setTheme(value)} aria-pressed={theme === value}>{label}</button>)}</div><button className="sign-out" onClick={() => { localStorage.removeItem('skillforge-auth'); setAuth(null); setProfileId(null); setRoadmap(null); }}>Sign out</button><div className="nav-avatar" title={auth.user.name}>{initials}</div></nav>
    <main id="top">
      <section className="hero"><div className="hero-copy"><p className="kicker">AI-POWERED CAREER PLANNING</p><h1>Build a career path<br />that feels <i>possible.</i></h1><p>Tell us what you know today. SkillForge turns it into a focused, practical path toward the role you want.</p><div className="hero-metrics"><span><b>01</b> clear starting point</span><span><b>02</b> focused next steps</span></div></div><div className="hero-card"><div className="pulse"><Icon name="spark" size={26} /></div><p>YOUR NEXT CHAPTER</p><strong>{profile.targetRole}</strong><div className="orbit o1" /><div className="orbit o2" /><div className="floating-chip chip-one">{profile.skills[0] || 'Your skill'}</div><div className="floating-chip chip-two">Future ready</div></div></section>
